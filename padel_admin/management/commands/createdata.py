   ...
   def handle(self, *args, **kwargs):
      ...
      for i in range(RESERVAS):
         for jug in jugadors_list:
            ...
            reserves = Reserva.objects.filter(pista=pista, data=data)
            for reserv in reserves:
               if (reserv.horaInici <= horaInici <= reserv.horaFinalitzacio or reserv.horaInici <= horaFinalitzacio <= reserv.horaFinalitzacio):
                  pista = random.choice(pistes)
         ...
         for i in range(random.choice(jugen)):
            ...
            if (quiReserva):
               ...
               for single in cobraments:
                  res2 = single.reserva
                  if (res2.data == res.data and (res2.horaInici <= res.horaInici <= res2.horaFinalitzacio or res2.horaInici <= res.horaFinalitzacio <= res2.horaFinalitzacio)):
                     sortir = True
                     break
               if (sortir == True):
                  continue
